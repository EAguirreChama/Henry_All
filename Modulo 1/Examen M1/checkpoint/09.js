...
BinarySearchTree.prototype.getHouseValues = function (side, total = 0) {
  ...
  if (side === "left") {
    if(this.left) {
      let Total = this.left.value
    return total = Total + this.left.getHouseValues(side, total)
    }
  }
  if (side === "right") {
    if (this.right) {
      let Total = this.right.value
    return total = Total + this.right.getHouseValues(side, total)
    }
  }
  return total
  // Termine el 9
